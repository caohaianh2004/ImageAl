...
struct ResultMultiFace: View {
    ...
    var restoreMultiface: MultiSFace
    ...
    var body: some View {
        ...
        .toast(isPresenting: $showAlertDowload) {
            AlertToast(type: .complete(.green), title: Optional("abc_download"), subTitle: Optional("abc_dowload_image"))
            ...
            if isHistory {
                Task {
                    ...
                    let imageUesr = ImageUser.Builder()
                        .setId(-1)
                        ...
                        .setType(4)
                        .build()
                    await userViewModel.addImage(imageUesr)
                }
                
                let request = MultiSFace(original: "", images: [])
                enhanceViewModel.cleanState()
                router.navigateTo(.result_multiface(imageAfter, nil, request, false, origin, false))
            }
        }
    }
